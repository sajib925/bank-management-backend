...
def payment_request(amount, user, deposit):
    settings = {'store_id': 'own671f4d7ec684f', 'store_pass': 'own671f4d7ec684f@ssl', 'issandbox': True}
    sslcz = SSLCOMMERZ(settings)
    ...
    post_body = {
        'total_amount': amount,
        'currency': "BDT",
        'tran_id': deposit.transaction_id,  # Use the deposit's transaction ID
        'success_url': f"{frontend_link}/transaction",
        'fail_url': f"{frontend_link}/fail",
        'cancel_url': f"{frontend_link}/cancel",
        'emi_option': 0,
        'cus_name': user.username,
        'cus_email': user.email,
        'cus_phone': "01740786762",
        'cus_add1': "Dhaka, Bangladesh",
        'cus_city': "Dhaka, Bangladesh",
